...
public class QueueQuantityCellEditor extends DefaultCellEditor {
    ...
    public QueueQuantityCellEditor(EventSpinnerInputChange event) {
        super(new JCheckBox());
        this.event = event;
        input = new JSpinner();
    ...
    public Component getTableCellEditorComponent(JTable table, Object value, boolean isSelected, int row, int column) {
        ...
        this.table = table;
        this.row = row;
        this.item = (DTO_InvoiceDetail) table.getValueAt(row, 10);
    ...
    private void inputChange() {
        ...
        if (quantity != item.getQuantity()) {
            ...
            item.setTotalprice(item.getOption().getImportprice() * quantity);
            table.setValueAt(df.format(item.getTotalprice()), row, 9);
            event.inputChanged();
        }
    }
}
