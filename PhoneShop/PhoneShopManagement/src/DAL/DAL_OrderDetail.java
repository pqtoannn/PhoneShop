...
public class DAL_OrderDetail extends DAL_Connection {
    ...
    public ArrayList <DTO_InvoiceDetail> getList(String id) {
        String query = "SELECT `optionid`, `quantity`, `totalprice`, `productid`, `option`.`color`, `color`.`name`, `product`.`name` as `prname`, `rom`, `ram`, `importprice`, `saleprice`, `remain`, `sold` FROM `orderdetail`, `option`, `color`, `product` WHERE `orderid` = '" + id + "' AND `optionid` = `option`.`id` AND `option`.`color` = `color`.`rgb` AND `product`.`id` = `option`.`productid`";
        ...
        try {
            ...
            while(rs.next()) {
                ...
                DTO_InvoiceDetail detail = new DTO_InvoiceDetail("", optionid, quantity, totalprice);
    ...
    public int insert(DTO_InvoiceDetail item) {
        String query = "INSERT INTO `orderdetail`(`orderid`, `optionid`, `quantity`, `totalprice`) VALUES (?, ?, ?, ?)";
        int rs = -1;
